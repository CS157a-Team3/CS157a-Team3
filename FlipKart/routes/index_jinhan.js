...
module.exports = {
    ...
    cardAdd: (req, res) => {
        res.render('cardadd', {message: "Create User"});
    },

    payment: (req,res) => {

        let cardnumber = req.body.cardnumber;
        let holdername = req.body.cardholdername;
        let expiry = req.body.expiry;
        let cvv = req.body.cvv;

        let numberCheck = "SELECT * FROM 'user' WHERE cardNum = '"+cardnumber+"'";
        ...
        db.query(emailCheck, function(error, results, fields)
        {
            if(results)
            {
                ...
                res.render('cardadd', {message: "Email already exists!"});
            }
            else
            {
                let useruuid = uuid.v4();
                let addQuery = "INSERT INTO user (cardNum, cardName, CVV, expDate) VALUES ('" + cardnumber + "', '" + holdername + "', '" + cvv + "', '" + expiry + "')";
                db.query(addQuery, (err, result) => {
                    if (err) throw err;
